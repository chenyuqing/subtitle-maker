...
- 2026-04-30：字幕换行器只能“重排”不能“截断”，任何 `max_chars` 规则都必须保证文本完整保留，否则会造成语义缺失。
- 2026-04-30：长句拆分会重新引入弱词起句（如 `and/that/is`）；句首修复必须在“拆分后”再跑一遍，不能只在拆分前修一次。
- 2026-04-30：每次中等及以上改动的执行计划都必须归档到 `/docs/plans/`，并使用四位编号递增（`0001`、`0002`...），避免上下文回滚后计划丢失。
- 2026-04-30：ASR 字幕质量问题优先在前置链路修复（分句 + chunk 边界去重），后置仅保留 O(n) 轻兜底；禁止“先产垃圾字幕，再重后处理”。
