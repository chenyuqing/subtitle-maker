...
- 2026-05-08：侧栏里像 `连通成功 @ host / model` 这种长状态文案不要直接做成可视主文本；应改成短状态词 + `title` 详情，否则窄侧栏会被横向撑坏。
- 2026-05-08：侧栏底部的辅助操作按钮如果属于“配置检查/测试”这类高频动作，优先放在卡片头部动作区，不要塞进正文区；正文区很容易因为首屏高度不够把按钮挤到折叠区外。
- 2026-05-08：OmniVoice 结果恢复不能只换主播放器媒体；批次选择框和结果附件列表也必须跟着显式刷新并保留当前选中 batch，否则用户会看到“视频切了但结果区还是旧批次”的假故障。
...
- 2026-05-08：OmniVoice 结果态如果已经生成 `final/dubbed_video_full.mp4` 和 `final/dubbed_final_full.srt`，5 号面板应优先直接加载成片视频和结果字幕，不要再绕单独音轨去拼结果；这样更稳，也更符合用户对结果回放的直觉。
- 2026-05-08：5 号面板结果预览不能默认假设 `Original Only` 一定有原字幕；如果当前项目没有原字幕，播放器 overlay 应在结果态把 `Original Only` 回退到结果字幕，否则用户会以为字幕没加载。
...
- 2026-05-08：当用户明确要求“不要动 Deepgram JSON 部分”时，导入优化必须单独落在 SRT 路径上；不能为了复用规则把 JSON 解析和 SRT 解析绑成同一个入口，否则会扩大回归面并违背用户约束。
- 2026-05-08：SRT 导入优化如果只做“保守地合并同 speaker 相邻短句”，效果通常很小；更实用的做法是按连续 speaker block 走 short-merge 目标时长重分句，否则用户会觉得“392 句只变成 384 句，没啥用”。
- 2026-05-08：当目标是“尽量一句完整的话，不要把句子拆到不同的行”时，导入顺序必须是“先按 speaker 合并，再按标点切句”；反过来先切句再合并，容易把一个完整句子拆碎后再拼回去，稳定性更差。
- 2026-05-08：SRT 导入不能把“短句合并”当成主规则，主规则应该是“speaker + 停顿分块 + 句末标点收敛”；只有像 `但`、`了。` 这种碎片才做修补，像 `No.` 这种完整短句不能被误归为碎片。
- 2026-05-08：当用户只上传 1 份 speaker 参考音时，不要继续沿用多人模式的“0 上传或全上传”思路，应该直接收敛成 single speaker 并走更简单的句子优先规则；只有 2 份及以上参考音才进入 multi speaker 校验。
- 2026-05-12：5 号面板缺失参考音补位时，`ref-voices/<lang>/male/` 和 `ref-voices/<lang>/female/` 子目录是权威来源；正确顺序是先判断缺失 speaker 性别，再只从对应目录随机选，不要反过来扫描全部候选文件再推断候选性别。
- 2026-05-12：缺失 speaker 的性别探测音频是运行时从原音频裁出来的 probe，不是人工整理的参考音目录；这条路径不能先信任目录/文件名提示，应该直接走“本地模型 -> pitch 兜底”。
