...
def resolve_omnivoice_checkpoint_path() -> Path:
    """解析 OmniVoice 的本地 checkpoint 路径。

    只接受本地目录，不再把 HuggingFace repo id 当成默认入口。
    如果环境变量给的是远端 repo 名，会自动回退到本机 checkpoints。
    """

    candidates = [
        os.environ.get("OMNIVOICE_MODEL"),
        os.environ.get("OMNIVOICE_MODEL_DIR"),
        str(_LOCAL_OMNIVOICE_CHECKPOINT),
    ]
    for candidate in candidates:
        ...
        if path.is_dir():
            return path.resolve()
    return _LOCAL_OMNIVOICE_CHECKPOINT.resolve()
...
def _load_model_sync():
    ...
    try:
        ...
        checkpoint_path = resolve_omnivoice_checkpoint_path()
        if not checkpoint_path.is_dir():
            raise FileNotFoundError(f"OmniVoice local checkpoint not found: {checkpoint_path}")
        load_asr = os.environ.get("OMNIVOICE_LOAD_ASR", "0").strip() == "1"
        _set_loading("loading_weights", f"Loading local TTS weights from {checkpoint_path} on {device}…")
        logger.info("Loading OmniVoice model from local checkpoint: %s on device: %s", checkpoint_path, device)
        _model = OmniVoice.from_pretrained(
            str(checkpoint_path), device_map=device, dtype=torch.float16, load_asr=load_asr,
        )
...
async def preload_model():
    ...
    try:
        checkpoint_path = resolve_omnivoice_checkpoint_path()
        if not checkpoint_path.is_dir():
            logger.warning("Preload skipped: local checkpoint not found at %s", checkpoint_path)
            return

        logger.info("Preloading TTS model from local checkpoint in background…")
        _last_used = time.time()
        async with _model_lock:
            if model is None:
                loop = asyncio.get_running_loop()
...
def get_model_status():
    ...
    result = {
        ...
        "loading": is_loading,
        "status": status,
        "checkpoint": str(resolve_omnivoice_checkpoint_path()),
    }
